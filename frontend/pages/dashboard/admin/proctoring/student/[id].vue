<template>
  <v-container fluid class="pa-6">
    ...
    <div class="d-flex align-center mb-6">
      ...
      <v-avatar :color="getAvatarColor(student?.name || '')" size="48" class="mr-4">
        <span class="text-h6 font-weight-bold text-white">{{ initials(student?.name || '') }}</span>
      </v-avatar>
      <div>
        <h1 class="text-h4 font-weight-bold mb-1 text-dark">{{ student?.name || 'Loading...' }}</h1>
        <p class="text-subtitle-1 text-medium-emphasis mb-0">Select an exam to view detailed proctoring logs.</p>
      </div>
      <v-spacer></v-spacer>
      <v-chip
        v-if="student?.isFlagged"
        color="error"
        size="large"
        variant="tonal"
    ...
    <!-- Empty State -->
    <v-card v-else-if="!student || student.attempts.length === 0" class="rounded-xl pa-12 text-center" elevation="0">
      ...
      <div class="text-body-2 text-disabled mt-1">This student has no proctored exam attempts.</div>
    </v-card>
      ...
      >
        ...
        <template v-slot:item.actions="{ item }">
          <v-btn
            ...
            :to="`/dashboard/admin/proctoring/${item.attempt_id}`"
          >
            View Logs
          </v-btn>
        </template>
...
// Removed: import { useApi } from '@/composables/useApi'; // Relies on Nuxt auto-imports
...
const attempts = computed<any[]>(() => {
  return student.value?.attempts || [];
});

const headers: any[] = [
  { title: 'Exam Name', key: 'exam_title', width: '35%' },
  { title: 'Course', key: 'course_title', width: '20%' },
  { title: 'Date & Time', key: 'date', width: '15%' },
...
onMounted(async () => {
  ...
  try {
    ...
    if (data && data.courses) {
      // Extract courseId and studentId from route.params.id (format: courseId_studentId)
      const [courseId, studentId] = (route.params.id as string).split('_');
      ...
      if (course) {
        ...
        const s = course.students.find((stu: any) => stu.id == studentId);
        if (s) {
          foundStudent = {
            id: s.id,
            name: s.name,
          ...
          s.attempts.forEach((a: any) => {
            foundStudent.attempts.push({
              ...a,
              course_title: courseTitle
            });
            if (a.highSeverityCount > 0) {
              foundStudent.isFlagged = true;
            }
          });
        }
      }
      ...
      if (foundStudent) {
        foundStudent.attempts.sort((a: any, b: any) => 
